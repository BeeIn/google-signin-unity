...
namespace Google
{
    ...
    public class GoogleSignIn
    {
        ...
        private ISignInImpl _implemWebPort;
        private ISignInImpl _implemCredsX;
        private ISignInImpl _currentImplem => UsePredefinedAccounts ? _implemCredsX : _implemWebPort;
        public bool UsePredefinedAccounts { get; set; }
        ...
        public GoogleSignIn(Action<string> openWebsiteImplem)
        {
            _implemWebPort = new Google.Impl.GoogleSignViaWebAndPort(Configuration, openWebsiteImplem);
#if UNITY_ANDROID && !UNITY_EDITOR
            _implemCredsX = new Google.Impl.GoogleSignInViaCredentialsX(Configuration);
#endif
        }

        public void EnableDebugLogging(bool flag)
        {
            _currentImplem.EnableDebugLogging(flag);
        }
        ...
        public Task<GoogleSignInUser> SignIn()
        {
            ...
            SignInHelperObject.Instance.StartCoroutine(_currentImplem.SignIn().WaitForResult(tcs));
            return tcs.Task;
        }
        ...
        {
            var tcs = new TaskCompletionSource<GoogleSignInUser>();
            _currentImplem.SignIn().WaitForResultAsync(tcs).ContinueWith((task) => { });
            return tcs.Task;
        }

        public Future<GoogleSignInUser> SignInFuture() => _currentImplem.SignIn();
        ...
        {
            var tcs = new TaskCompletionSource<GoogleSignInUser>();
            SignInHelperObject.Instance.StartCoroutine(_currentImplem.SignInSilently().WaitForResult(tcs));
            return tcs.Task;
        }
        ...
        {
            var tcs = new TaskCompletionSource<GoogleSignInUser>();
            _currentImplem.SignInSilently().WaitForResultAsync(tcs).ContinueWith((task) => { });
            return tcs.Task;
        }

        public Future<GoogleSignInUser> SignInSilentlyFuture() => _currentImplem.SignInSilently();
